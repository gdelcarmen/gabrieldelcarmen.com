...
const CVHighlightsSection = ({ isVisible }: SectionProps) => (
  ...
  >
    <div className="mb-6 flex flex-col gap-2 border-b border-gray-100 pb-4 md:flex-row md:items-end md:justify-between">
      <div>
        <p className="text-xs font-medium uppercase tracking-[0.2em] text-blue-700">Complete CV overview</p>
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      {highlights.map((item) => (
        <article key={item.label} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          <p className="mt-2 text-sm font-light leading-6 text-slate-600">{item.detail}</p>
        </article>
      ))}
    </div>
  </section>
